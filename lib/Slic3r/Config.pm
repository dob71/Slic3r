...
our $Options = {
    ...
    'gcode_flavor' => {
        ...
        cli     => 'gcode-flavor=s',
        type    => 'select',
        values  => [qw(reprap x2 teacup makerbot mach3 no-extrusion)],
        labels  => ['RepRap (Marlin/Sprinter)', 'RepRap X2', 'Teacup', 'MakerBot', 'Mach3/EMC', 'No extrusion'],
        default => 'reprap',
    },
...
END
    ...
    'extruder_set_gcode' => {
        label   => 'Extruder change G-code',
        tooltip => 'This custom code is inserted when setting the active extruder. The string "[extruder]" is replaced with 0-based number of the extruder being set as active. Note that you can use placeholder variables for all Slic3r settings, so you can put a "M104 S[first_layer_temperature]" command wherever you want.',
        cli     => 'extruder-set-gcode=s',
        type    => 's',
        multiline => 1,
        full_width => 1,
        height  => 50,
